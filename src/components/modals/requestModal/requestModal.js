...
class RequestModalForm extends Component {
        ...
        if (userInfo !== {}) {
          let data = {
            ...
            tripRequest: {
              ...
              client: !this.state.newClient ? values.client : null,
              tripType: "Drop",
            ...
            client: this.state.newClient
              ? {
                  name: values.clientName ? values.clientName : null,
                  primaryNumber: values.clientNumber
                    ? values.clientNumber
                    : null,
                  secondaryNumber: null, //not required
                  email: null,
                  //add company make sure you are field same as in cleint section
                  type: "Guest",
                  address: {
                    street: values.clientAddress ? values.clientAddress : null,
                    city: values.clientCity ? values.clientCity : null
                  },
                  preferedLocations: null, //do no remove make i send you null
                  profilePicture:
                    "https://cdn.pixabay.com/photo/2015/03/04/22/35/head-659652__340.png",
                  poc: null, //do no remove make i send you null
  ...
  beforeFormTwo = () => {
    let tripDataArray = [];
    let counter = 1;
    this.state.VehicleData.map((val, ind) => {
    ...
    this.setState({ tripData: tripDataArray });
  };
  // this will call on Trip form added
  handleSubmitTrip = e => {
    e.preventDefault();
    this.props.form.validateFields((err, values) => {
      if (!err) {
        let assignedToArray = [];
        this.state.tripData.map((val, ind) => {
          if (val.type === "driver") {
            assignedToArray.push({
              type: "Driver",
              driver: values["assign" + val.key],
              vehicle: val.driverVehicleId
            });
          } else if (val.type === "broker") {
            assignedToArray.push({
              type: "Broker",
              broker: values["assign" + val.key],
              brokerVehicleType: values["brokerVehicleType" + val.key],
              brokerVehicle: values["brokerVehicle" + val.key],
              brokerDriver: values["brokerDriver" + val.key]
            });
          }
        });
        ...
        if (userInfo !== {}) {
          let data = {
            assignedLabour: null,
            costOfLabour: values.costLab ? values.costLab : null,
            tripRequestId: newTripRequest._id,
            tripStatus: "Upcoming",
            userId: userInfo.user._id,
            accountType: "Transporter",
            blt: values.bltNumber ? values.bltNumber : null,
            rateQoute: null,
            advancePaid: values.advancepPayment ? values.advancepPayment : null,
            bltReceivingDate: values.bltDate
              ? moment(values.bltDate).toISOString()
              : null,
            bltDeliveredBy: values.bltDeli ? values.bltDeli : null,
            additionalDescription: values.descriptionL
              ? values.descriptionL
              : null,
  ...
  // this function is for render Client Selector or New Clients Fields
  renderSelectClient = () => {
        ...
        <div style={{ display: "flex", flexDirection: "row" }}>
          ...
          <Col span={18}>
            <div style={{ display: "flex", flexDirection: "row" }}>
              ...
              <span style={{ marginLeft: 10, marginTop: 8 }}>- or -</span>
  ...
  // this fuction is for to render Form one or Two i.e Trip Request or Trip
  renderForm = () => {
    const { getFieldDecorator } = this.props.form;
    const {
      onChangeRequestModalVisible,
    ...
    if (this.state.form === "1st") {
      ...
      return (
        <div>
          <Form onSubmit={this.handleSubmitTrip} spellCheck={true}>
            <ApnaDivider text={"Payment"} />
            <div style={{ display: "flex", flexDirection: "row" }}>
              <Col span={6} style={{ paddingTop: 5 }}>
                <span
                  style={{ fontSize: sizes.h3, color: textColor.text_dark }}
              ...
              <Col span={18}>
                <div style={{ display: "flex", flexDirection: "row" }}>
                  ...
                  <Form.Item>
                    {getFieldDecorator("advancepPayment", {})(
                      <InputNumber
            ...
            <Table
              ...
              expandedRowRender={text =>
                text.desc ? (
                  ...
                  >
                    <Form.Item>
                      {getFieldDecorator("brokerVehicleType" + text.key, {
                        rules: [
                          {
                            required: true,
                    ...
                    <Form.Item>
                      {getFieldDecorator("brokerVehicle" + text.key, {
                        rules: [
                          {
                            required: true,
                    ...
                    <Form.Item>
                      {getFieldDecorator("brokerDriver" + text.key, {})(
                        <Input
                          type="text"
                          style={{ width: 150 }}
            ...
            <div style={{ display: "flex", flexDirection: "row" }}>
              ...
              <Col span={18}>
                <div style={{ display: "flex", flexDirection: "row" }}>
                  <Form.Item>
                    {getFieldDecorator("bltNumber", {})(
                      <Input
                        style={{ width: 190, marginRight: 10 }}
                        placeholder="BLT Number (Optional)"
                        type="text"
                      />
                    )}
                  </Form.Item>
                  <Form.Item>
                    {getFieldDecorator("bltDate", {})(
                      <DatePicker
                        style={{ width: 190, marginLeft: 10 }}
                        placeholder="BLT Date"
                      />
                    )}
                  </Form.Item>
                </div>
            ...
            <div style={{ display: "flex", flexDirection: "row" }}>
              ...
              <Col span={18}>
                <Form.Item>
                  {getFieldDecorator("bltDeli", {})(
                    <Input
                      style={{ width: "100%", marginRight: 10 }}
                      placeholder="Enter BLT Delivered By (Optional)"
                      type="text"
                    />
                  )}
                </Form.Item>
    ...
    if (!this.state.additional) {
      return (
        <Row style={{ marginTop: -8, marginBottom: 15 }}>
          ...
          >
            <Icon type="plus" size="small" /> Additional Options
          </span>
        </Row>
      );
    } else {
      return (
        <Col span={24}>
          <Row style={{ marginTop: -8, marginBottom: 15 }}>
            ...
            >
              <Icon type="minus" size="small" /> Additional Options
            </span>
          </Row>
          <div style={{ display: "flex", flexDirection: "row" }}>
            <Col span={6} style={{ paddingTop: 5 }}>
            ...
            <Col span={18}>
              <Form.Item>
                {getFieldDecorator("costLab", {})(
                  <InputNumber
                    style={{ width: "100%", marginRight: 10 }}
                    placeholder="Enter Cost of Labours"
                  />
                )}
              </Form.Item>
            </Col>
          ...
          <div style={{ display: "flex", flexDirection: "row" }}>
            ...
            <Col span={18}>
              <Form.Item>
                {getFieldDecorator("descriptionL", {})(
                  <Input.TextArea
                    style={{ width: "100%", marginRight: 10 }}
                    placeholder="Write other relevent/ important information (Optional)"
                  />
        ...
        <div style={{ display: "flex", flexDirection: "row" }}>
          ...
          >
            <ModalCompoentLeftComtent
              mainHeading={"Add Trip Request"}
              extraText={"Add Trip Details"}
              Image={Images.TRIP}
            />
          </div>
          <div
  ...
  componentDidMount() {
    ...
    if (data) {
      ...
      this.props.form.setFields({
        client: {
          value: data.clientInformation._id ? data.clientInformation._id : ""
        },
        gType: {
...
const RequestModal = Form.create()(RequestModalForm);
...
const mapDispatchToProps = dispatch => ({
  ...
  onAddTripPush: data => dispatch(ActionsGlobal.ON_TRIP_ADDED_PUSH_TO_ALL(data))
});
